...
fun layout ast =
    case ast of
        Var (t, v) => paren (colon (Id.layout v, Type.layout t))
      | VarD (t, v, _) => paren (colon (Id.layout v, Type.layout t))
      ...
      | AMap (_, e1, e2) => paren (spaces ["ExtendedList.map", layout e1, layout e2])
      | AFoldl (_, e1, e2, e3) => paren (spaces ["ExtendedList.foldl", layout e1, layout e2, layout e3])
      | AMapi (_, e1, e2) => paren (spaces ["ExtendedList.mapi", layout e1, layout e2])
      | AFoldli (_, e1, e2, e3) => paren (spaces ["ExtendedList.foldli", layout e1, layout e2, layout e3])
      | ANth (_, e1, e2) => paren (spaces ["ExtendedList.nth", paren (comma (layout e1, layout e2))])
